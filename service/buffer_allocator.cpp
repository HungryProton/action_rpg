...
namespace game{
	...
	void BufferAllocator::ClearMemory(){
	...
	GLuint BufferAllocator::RegisterData(std::multimap<int, std::vector<float> > data, std::vector<unsigned int> indices, int* offset){
	...
	GLuint BufferAllocator::CreateAndBindVertexArray(){
		GLuint vertexArray;
	...
	void BufferAllocator::BindBuffer(GLuint type, int data_size){
	...
	int BufferAllocator::CountDataToRegister(std::multimap<int, std::vector<float> > data){
	...
	int BufferAllocator::GetElementSize(int type){
		switch(type){
			case VERTEX_ARRAY:
				return 3;
			case COLOR_ARRAY:
	...
	void BufferAllocator::FillIndexBuffer(std::vector<unsigned int> indices, int* offset){
